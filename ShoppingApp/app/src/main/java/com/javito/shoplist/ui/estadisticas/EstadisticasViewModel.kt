...
class EstadisticasViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val ingresos: LiveData<List<Ingreso>> = _selectedMonth.switchMap { m ->
        repository.getIngresosByMonth(m).asLiveData()
    }
    ...
    val sourceBreakdown: LiveData<Map<String, Double>> = MediatorLiveData<Map<String, Double>>().apply {
        addSource(ingresos) { list ->
            value = list.groupBy { it.source }.mapValues { (_, items) -> items.sumOf { it.amount } }
        }
    }
